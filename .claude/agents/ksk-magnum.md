...
- `learning-notes.md` at the client root — **optional**; its absence is normal and not a finding. When present, read **only the unhandled bullets**, `- [ ] **title** — detail` (or a legacy bullet with no checkbox at all, which counts as unhandled too); a handled bullet, `- [x] **title** — detail`, has already been folded into `CLIENT.md`/`coa_usage.json` (or judged not worth folding in) and must be ignored
...
9. **Draft COA conventions.** Propose a few `coa_conventions` — line-pattern → `account_code` defaults drawn *only from codes that exist in `coa.csv`* — for the recurring ambiguous buckets you can foresee (e.g. "PVC/paint/hardware from home-improvement retailers → `510111` ซื้อวัตถุดิบ, assuming production input"). Mark each convention's assumption so the human can veto it. Never invent codes. A **revenue-family** convention — which income account the client's own sales book to (construction `410401` vs service `410201` vs product `410101`) — is especially prone to being a folder-name guess: propose it only from `coa_usage.json` income history or a sampled income document that names the revenue type. When it rests on `business_nature` or the `รายได้` folder alone, set that convention's `confidence: low` and add the split as a concrete `needs_confirmation` question — poirot down-weights an `assumption`-only convention and flags it, so an over-confident revenue default here silently mis-codes every income row. `learning-notes.md`, when present, is another source of candidate conventions — but each unhandled bullet is a **proposal an AI wrote and a human kept**, not a confirmed rule: weigh it exactly the way you already weigh an `assumption`-marked convention (it may earn a `coa_conventions` entry for the human to confirm) and never let a note alone justify `confidence: high`.
...
- Write only `CLIENT.md` and (when converting) `coa.csv`. Touch nothing else — in particular, never create or edit `ข้อมูลระบบ/_pages/dispositions.yaml`, any `ข้อมูลระบบ/_pages/fragments/*.yaml`, `ข้อมูลระบบ/_pages/ledger.yaml`, or any other disposition/ledger file, under any schema name. Those files are parent-only; a leaf agent has no standing to write them regardless of what schema it uses. Your entire contribution to Page Disposition is one reported line in your reply digest — never a file on disk.
- **Reply = digest, artifacts = disk.** The full profile lives in `CLIENT.md`; never paste it back. Reply to the parent with a thin digest only: the `CLIENT.md` path, the `coa_csv` status (`present`/`converted_from_workbook`/`missing`), the key provisional facts (client name, `vat_registered`, `business_nature`), a named `page_disposition:` line reporting the file-level exclusion for any context file you consumed (file path + reason `context_file` — text in the digest, never a file on disk), and the full `needs_confirmation` list (the parent needs every item to resolve by policy).
