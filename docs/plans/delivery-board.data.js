window.KSK_BOARD = {
  ...
  "tasks": [
    ...
    },
    {
      ...
      "notes": "run #1 (20260713-1750) DIED @ Stage 2 (7/20 interp). ROOT CAUSE: orchestrator dispatched the interpret wave via the async background Workflow tool (per orchestration.md) then yielded 'standing by, harness will re-invoke me' — headless `claude -p` has NO re-invoke loop, so it returned exit-0 and orphaned the wave. Stage 0/1 (foreground Agent) completed fine; both T01+T02 fixes VALIDATED in situ before death. FIX: waves must be FOREGROUND Agent(run_in_background:false) under headless (awaited in-turn). Applying orchestration.md headless subsection + re-running fresh as run #1b. See /tmp scratchpad t05-findings.md."
    },
    {
      "id": "T06",
