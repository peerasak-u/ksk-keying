...
Instructions for coding agents working in this repo. (`CLAUDE.md` is a symlink to this file.)
...
## `platform-mock/` — design mock, not product code

`platform-mock/index.html` is a self-contained clickable mock of the proposed KSK office
platform (see its README). It is a design artefact for review only: no build, no deps, not
wired to the pipeline, and never deployed with the skill. Do not import from it, and leave it
out of the Dropbox sync set above.
