...
// coverage. Covers both the "confirm" branch (ticket #44) and the
// "bring_back" branch (ticket #46, Build: excluded-review bring-back +
// repairRun).
...
describe("applyDecision", () => {
	...
	test("bring_back replaces the entry with disposition used, declared_by human, no reason/duplicate_of/note", () => {
		const existing = [excluded("a.pdf", 1, "duplicate", "agent", "b.pdf#p1")];
		const result = applyDecision(existing, "a.pdf#p1", "bring_back");
		expect(result.ok).toBe(true);
		if (!result.ok) return;
		expect(result.entries).toHaveLength(1);
		expect(result.entries[0]).toEqual({
	...
	test("bring_back leaves every other entry untouched", () => {
		const existing = [excluded("a.pdf", 1, "duplicate", "agent"), excluded("b.pdf", 2, "blank_or_separator", "agent")];
		const result = applyDecision(existing, "a.pdf#p1", "bring_back");
		expect(result.ok).toBe(true);
		if (!result.ok) return;
	...
	test("bring_back rejects when the target unit key isn't found", () => {
		const existing = [excluded("a.pdf", 1, "duplicate", "agent")];
		const result = applyDecision(existing, "missing.pdf#p1", "bring_back");
		expect(result.ok).toBe(false);
	});

	test("bring_back rejects when the entry is already declared_by human", () => {
		const existing = [excluded("a.pdf", 1, "duplicate", "human")];
		const result = applyDecision(existing, "a.pdf#p1", "bring_back");
		expect(result.ok).toBe(false);
	});

	test("bring_back rejects when the entry has no reason (reason-required-when-excluded, re-checked at write time)", () => {
		const existing: DispositionEntry[] = [
			{ file: "a.pdf", page: 1, sheet: null, disposition: "excluded", declared_by: "agent" },
		];
		const result = applyDecision(existing, "a.pdf#p1", "bring_back");
	...
	test("bring_back rejects when the entry's disposition is used, not excluded", () => {
		const existing: DispositionEntry[] = [
			{ file: "a.pdf", page: 1, sheet: null, disposition: "used", declared_by: "agent" },
		];
		const result = applyDecision(existing, "a.pdf#p1", "bring_back");
	...
	test("bring_back matches file-level (bare) unit keys", () => {
		const existing = [excluded("coa.xlsx", null, "context_file", "agent_policy")];
		const result = applyDecision(existing, "coa.xlsx", "bring_back");
		expect(result.ok).toBe(true);
	});
});
