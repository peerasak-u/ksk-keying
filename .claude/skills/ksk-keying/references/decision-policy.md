...
12. **A bank statement is a period ledger, never a transaction member** — a source interpreted as `doc_kind: bank_statement` (account number, statement period, opening/closing balance, a `transactions` list) covers a whole month and is evidence for *many* transactions at once, so it can never be a member of another document's cluster: it always gets its **own single-segment transaction** and its **own `bank_statement` review bucket**, no matter how many of its lines match documents in the same month. A statement line that does match a document is recorded as a payment **reference** in that document's own cluster evidence (date + amount + counterparty in prose), never as a merge. This holds even when the statement's lines match every bookable document in the month — *especially* then. Why it is a rule and not a judgment call: `links.yaml` gives each document exactly one cluster, so folding a statement into one document's cluster silently drags every other document that matched a statement line into that same cluster (real incident, 339/69-03: four unrelated bookables fused into one transaction, the statement lost its own review page entirely, and its single page was then re-rendered inside all four รายได้/ค่าใช้จ่าย buckets stamped with four different documents' numbers — while the same client's 69-04, whose statement lines happened to match nothing, came out correct). Amounts were never double-booked in that incident; the damage is the review surface and the statement never being reviewed as a statement.

Rules 3–5 and 9 resolve most of what segmentation (`ksk-columbo`) flags; rules 6–8 and 10
resolve most of what interpretation surfaces; rule 12 binds linking (`ksk-sherlock`). A question no rule covers that does **not**
