...
The parent's final report to the human **must list every agent-declared Exclusion Declaration** from the `final`-gate output (the "AGENT-PROPOSED EXCLUSIONS" section / `agent_declared_exclusions` in `_pages/ledger.yaml`) as an open review point — never silently accept an agent's exclusion as final. A human confirms one by re-recording that same entry in `_pages/dispositions.yaml` with `declared_by: human`.
