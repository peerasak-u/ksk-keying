...
0. Context files, ensured by `ksk-magnum` at Stage 0:
   ...
   - `coa.csv` — **required** (poirot's only source of account codes). If absent, `ksk-magnum` converts it from the client's `ผังบัญชี` chart-of-accounts workbook via `bun run --cwd tools/ksk coa-to-csv`. If no workbook exists either, the run is blocked until the client supplies a chart of accounts.
...
7. `_doc_groups/<category>/<vat>/review.html` + `assets/` at each bucket root — generated by `bun run --cwd tools/ksk review-groups`, one interactive page per bucket with PEAK XLSX export (saved as `peak_import_<bucket>.xlsx` beside review.html). The reviewer previews the **real source document** inline — the generator points each page at its `source_src` file (PDF rendered via `<iframe ...#page=N>` opened to `source_page`, images inline, xlsx as an open-file link), so every `review-data.json` page must carry a valid `source_src`/`source_page`.
...
```bash
bun run --cwd tools/ksk review-groups -- --force "${clientPath}"
```
