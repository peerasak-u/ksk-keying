...
3. Assign `confidence` (`high` | `medium` | `low`) and a short `reason` citing the evidence (matched tax_id, matched keyword, or account-name reasoning), **written in Thai** — the reviewer reading `review.html` is a Thai bookkeeper, so the sentence must read naturally in Thai, not English or code-mixed prose.
...
7. **WHT sanity on service expenses**: rent, professional fees, transport, and repair services from juristic sellers customarily carry withholding — when such a line's interpretation shows no WHT evidence, keep your account mapping but set `needs_review: true` with `review_flags` entry `wht_expected?` **and a Thai `reason` sentence** naming the line(s), the seller type, and what's missing (e.g. `"รายการค่าเช่าและค่าบริการเข้าข่ายต้องหักภาษี ณ ที่จ่าย เนื่องจากจ่ายให้ผู้ขายที่เป็นนิติบุคคลและเป็นค่าใช้จ่ายประเภทที่ต้องหัก แต่ในเอกสารไม่ปรากฏรายการหัก ณ ที่จ่าย ไม่มียอดภาษีหัก และไม่มีหนังสือรับรองการหักภาษี โปรดตรวจสอบก่อนบันทึกด้วยยอดเต็ม"`) so the reviewer checks before keying at full amount.
...
```json
{
  ...
  "lines": [
    { "line_index": 0, "account_code": "510111", "sub_code": "", "account_name_th": "ซื้อวัตถุดิบ",
      "confidence": "high", "reason": "ตรงกับคำสำคัญใน coa_usage", "needs_review": false }
  ],
  "questions_for_user": []
}
```
